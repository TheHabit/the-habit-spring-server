...
public class RecordController {
    ...
    @PostMapping("")
    public ResponseEntity<ResponseDTO> insertRecord(@RequestBody Record record){

        System.out.println("record = " + record);
        
        try{
            return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "독서기록 입력 성공", recordService.insertRecord(record)));
        } catch(Exception e){
